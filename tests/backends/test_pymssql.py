...
from classic.migrations.backends.pymssql import PyMSSQLBackend

from tests.conftest import get_credentials

_env = get_credentials("MS_SQL_")
...
def _mssql_available() -> bool:
    ...
    try:
        conn = pymssql.connect(
            server=_env_value("host"),
            port=str(int(_env_value("port"))),
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "SQL Server unavailable: set MS_SQL_DATABASE_HOST, MS_SQL_DATABASE_PORT, "
        "MS_SQL_DATABASE_NAME, MS_SQL_DATABASE_USER, MS_SQL_DATABASE_PASSWORD"
    ),
)


@pytest.fixture
def backend() -> Generator[PyMSSQLBackend, None, None]:
    b = PyMSSQLBackend(
...
def test_list_tables_empty(backend: PyMSSQLBackend) -> None:
    assert backend.migration_table not in backend.list_tables()


def test_create_migration_table(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    assert backend.migration_table in backend.list_tables()


def test_mark_applied_inserts_event(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_unmark_appends_rolled_back_event(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    backend.mark("0001.init", "APPLIED")
...
def test_reapply_makes_migration_applied_again(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    backend.mark("0001.init", "APPLIED")
    backend.mark("0001.init", "ROLLED_BACK")
    backend.mark("0001.init", "APPLIED")
...
def test_pending_status(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_rolled_back_migration_has_correct_status(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_ensure_migration_table_creates(backend: PyMSSQLBackend) -> None:
    backend.ensure_migration_table()
    assert backend.migration_table in backend.list_tables()


def test_ensure_migration_table_idempotent(backend: PyMSSQLBackend) -> None:
    backend.ensure_migration_table()
    backend.ensure_migration_table()
    assert backend.migration_table in backend.list_tables()


def test_copy_versions(backend: PyMSSQLBackend) -> None:
    backend.execute(
...
def test_lock_acquire_release(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_transaction_commit(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_transaction_rollback(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_disable_transactions_noop(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
...
def test_quote_identifier(backend: PyMSSQLBackend) -> None:
    assert backend.quote_identifier("my_table") == '"my_table"'


def test_quote_identifier_with_quotes(backend: PyMSSQLBackend) -> None:
    assert backend.quote_identifier('test"table') == '"test""table"'


def test_execute_cursor(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    backend.mark("0001.init", "APPLIED")
...
def test_migration_history_returns_all_events(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    backend.mark("0001.a", "APPLIED")
    backend.mark("0002.b", "APPLIED")
    backend.mark("0001.a", "ROLLED_BACK")
...
def test_migration_history_no_comment_column(backend: PyMSSQLBackend) -> None:
    backend._create_migration_table()
    backend.mark("0001.a", "APPLIED")
    history = backend._migration_history()
...
def test_migration_schema_creates_schema() -> None:
    ...
    try:
        ...
        b.execute('DROP TABLE IF EXISTS "migrations_schema_test"."migrations"')
        b.execute('DROP SCHEMA IF EXISTS "migrations_schema_test"')
        b.close()


class TestPyMSSQLLifecycle:
